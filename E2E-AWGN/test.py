...
for Eb_N0_dB in range(0,15):
    ...
    model_output = Conv1D(filters=2 ** k, strides=1, kernel_size=1, padding = 'same' , name='d_13', activation='softmax')(d)
    ...
    model.load_weights('/content/drive/MyDrive/project/model_LBC_4_10_1_9dB AWGN.h5')
...
plt.xlabel('Eb/N0')
plt.ylabel('BER')
plt.title('BER for the encoder/decoder model(No channel GAN)')
plt.grid(b = True)
plt.savefig('BER.png')
plt.show()
